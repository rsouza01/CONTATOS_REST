...
(function (angular, factory) {
    ...
    psm.provider('exDialog', [function () {
            ...
            var animationEndSupport = false, animationString = 'animation', keyframeprefix = '', domPrefixes = 'Webkit Moz O ms Khtml'.split(' '), pfx = '', testElem = document.createElement('div');
            if (testElem.style.animationName !== undefined) {
            ...
            this.$get = ['$document', '$templateCache', '$compile', '$q', '$http', '$rootScope', '$timeout', '$window', '$controller', function ($document, $templateCache, $compile, $q, $http, $rootScope, $timeout, $window, $controller) {
                    ...
                    var errorTemplate = "<div style='padding:15px; margin-right:15px; padding-bottom:15px; margin-left:15px;'><b>Error</b>:<br/><span>{#message#}<span></div>";
                    var pubMembers = {
                        ...
                        */
                        ...
                        open: function (prms) {
                            ...
                            function loadTemplate(template) {
                                if (!template || template == undefined || !angular.isString(template))
                                    return 'No dialog template';
                        ...
                        close: function (id, value, closeImmediateParent, closeAllDialogs) {
                            ...
                            if (closeImmediateParent === void 0) { closeImmediateParent = false; }
                            if (closeAllDialogs === void 0) { closeAllDialogs = false; }
                            if (closeAllDialogs) {
            ...
            return function (scope, element) {
                ...
                scope.$watch(function () {
                    return {
                        ...
                        'sy': window.scrollY,
                        'ch': element[0].children[1].offsetHeight,
                    };
                }, function (newValue) {
                    if (!hasDragged) {
...
//# sourceMappingURL=ngExDialog.js.map