//TypeScript compatible versions: 1.8.3.0 to 2.1.5.0.
//Angular compatible versions: 1.3x - 1.5x. 
(function (angular, factory) {
   return factory(angular);   
} (angular || null, function (angular) {
    "use strict";
    var psm = angular.module('ngExDialog', []);
    var dialogDefaults: any = {};
    var hasDragged = false;

    //Main configurable dialog provider.
    psm.provider('exDialog', [function () {
        //Check CSS animation support.
        var animationEndSupport: boolean = false,
            animationString: string = 'animation',
            keyframeprefix: string = '',
            domPrefixes: Array<string> = 'Webkit Moz O ms Khtml'.split(' '),
            pfx: string = '',
            testElem: any = document.createElement('div');

        if (testElem.style.animationName !== undefined) { animationEndSupport = true; }

        if (animationEndSupport === false) {
            for (var i = 0; i < domPrefixes.length; i++) {
                if (testElem.style[domPrefixes[i] + 'AnimationName'] !== undefined) {
                    pfx = domPrefixes[i];
        ...
        var animationEndEvent : string = 'animationend webkitAnimationEnd oanimationend mozAnimationEnd MSAnimationEnd ';
        var scope : any;
        var closeByEvents : any;
        var seqNumber : number = 0;
        var dialogCount : number = 0;
        var deferList : any = {};

        //Default settings. Will be overwritten by settings from app config level.
        ...
        this.$get = ['$document', '$templateCache', '$compile', '$q', '$http', '$rootScope', '$timeout', '$window', '$controller', function ($document, $templateCache, $compile, $q, $http, $rootScope, $timeout, $window, $controller) {
            ...
            var errorTemplate: string = "<div style='padding:15px; margin-right:15px; padding-bottom:15px; margin-left:15px;'><b>Error</b>:<br/><span>{#message#}<span></div>";
                ...
                */
                ...
                openMessage: function (prm1, message, title, icon) {
                    ...
                    if (params.scope && angular.isObject(params.scope)) {
                        ...
                        //Check for dev code only.
                        params.template = errorTemplate.replace("{#message#}", "No current scope in parameters");                        
                    }
                ...
                openConfirm: function (prm1, message, title, icon) {
                    ...
                    if (params.scope == undefined || !angular.isObject(params.scope)) {
                        //Check for dev code only.
                        params.template = errorTemplate.replace("{#message#}", "No current scope in parameters");
                        var dialog = pubMembers.open(params);
                        return dialog;
                    }
                    else {
                ...
                openPrime: function (params) {
                    ...
                    if (params.scope == undefined || !angular.isObject(params.scope)) {
                        ...
                        params.template = errorTemplate.replace("{#message#}", "No template defined");
                    }
                    return pubMembers.open(params);
                },

                //Base dialog, usually not directly called.
                open: function (prms : any) {
                    var dialogObj : any, dialogParentObj: any;

                    var params : any = angular.copy(dialogDefaults);
                ...
                close: function (id : string, value : any = undefined, closeImmediateParent: boolean = false, closeAllDialogs: boolean = false) {
                    if (closeAllDialogs) {
        ...
        return {
            ...
            link: function (scope : any, elem) {
                ...
                if (scope.dialogProp.type == 'confirm') {
                    ...
                    //Explicit hide message dialog from DOM. 
                    //Without this, Chrome still shows actionButton for message dialog as child.
                    //IE works fine without this.
